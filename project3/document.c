...
int replace_text(Document *doc, const char *target, const char *replacement) {
    int para = 0, line = 0, target_len = strlen(target), repl_len = strlen(replacement),
        line_len = 0, beg_len = 0, repeat = 0;    
    char new_line[MAX_STR_SIZE + 1] = "", *found = NULL, *beginning = NULL;
    ...
    for (para = 0; para < doc->number_of_paragraphs; para++) {
        for (line = 0; line < doc->paragraphs[para].number_of_lines; line++) {
            ...
            if (found) {
                beginning = doc->paragraphs[para].lines[line];
                while (found) {
                    if (repeat == 0) {
                        beg_len = line_len - strlen(found);
                    } else {
                        line_len = strlen(beginning);
                        beg_len = line_len - strlen(found);
                    }
                    strncat(new_line, beginning, beg_len);
                    strcat(new_line, replacement);
                    beginning += beg_len + target_len;
                    found += target_len;
                    found = strstr(found, target); /* goes to next instance of target past current target*/
                    if (found) { /* if another instance was found*/
                        repeat = 1;
                    }
                }
                strcat(new_line, beginning);
                strcpy(doc->paragraphs[para].lines[line], new_line);
            }
           strcpy(new_line, "");
        }
    }
